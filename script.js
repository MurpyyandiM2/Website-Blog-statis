 ...
 * 
 * ✅ FIXED: Syntax errors, template literals, video support
 * ✅ COMPATIBLE: Firebase SDK v10.7.1 (Compat Mode)
 */
...
const FirebaseService = {
        ...
        try {
            ...
            if (files.video) {
                if (files.video.size > MAX_VIDEO_SIZE) {
                    throw new Error('Ukuran video melebihi batas 100MB');
                }
    ...
    updateTask: async (taskId, updates, files) => {
        try {
            ...
            if (files && files.video) {
                const oldTask = appData.tasks.find(t => t.id === taskId);
                if (oldTask?.videoFile?.path) {
                    await FirebaseHelper.deleteFile(oldTask.videoFile.path);
                }
    ...
    deleteTask: async (taskId) => {
        ...
        try {
            ...
            if (task?.videoFile?.path) {
                await FirebaseHelper.deleteFile(task.videoFile.path);
            }
...
function renderCertificates(certificates) {
    ...
    grid.innerHTML = certificates.map(cert => {
        ...
        const title = cert.title || 'Tanpa Judul';
        const issuer = cert.issuer || '';
        const year = cert.year || '';
        const icon = cert.icon || 'fa-certificate';
        
        return `
            <div class="certificate-card" onclick="showCertificateDetail('${certId}')">
...
async function handleAddTask(event) {
    ...
    try {
        ...
        const files = {
            ...
            video: document.getElementById('taskVideoFile')?.files[0] || null
        };
...
function createTaskCard(task) {
    ...
    // Build attachment badges
    let attachmentBadges = '';
    if (task.pdfFile) {
        attachmentBadges += '<span class="attachment-badge pdf"><i class="fas fa-file-pdf"></i> PDF</span>';
    }
    if (task.pptFile) {
        attachmentBadges += '<span class="attachment-badge ppt"><i class="fas fa-file-powerpoint"></i> PPT</span>';
    }
    if (task.videoFile) {
        attachmentBadges += '<span class="attachment-badge video"><i class="fas fa-video"></i> Video</span>';
    }
    
    return `
        <article class="task-card ${statusClass}" data-category="${category}" data-id="${taskId}">
            <div class="task-header">
                <span class="task-course">${course}</span>
                <span class="task-date">${dateStr}</span>
            </div>
            <h3 class="task-title">${title}</h3>
            <p class="task-description">${description}</p>
            
            ${attachmentBadges ? `<div class="task-attachments" style="display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap;">${attachmentBadges}</div>` : ''}

            <div class="task-footer">
                <span class="task-status ${statusClass}">
                    <i class="fas ${statusIcon}"></i> ${statusText}
                </span>
                <button class="btn btn-small" onclick="showTaskDetail('${taskId}')">
        ...
        return `
            <div class="admin-list-item">
                ...
                <div class="admin-list-item-actions">
                    <button class="btn-icon" onclick="deleteTask('${taskId}')" title="Hapus">
...
function loadAdminCertsList() {
    ...
    list.innerHTML = certificates.map(cert => {
        const certId = cert.id || '';
        const title = cert.title || 'Untitled';
        const issuer = cert.issuer || '-';
        const year = cert.year || '-';
        ...
        return `
            <div class="admin-list-item">
                ...
                <div class="admin-list-item-actions">
                    <button class="btn-icon" onclick="deleteCertificate('${certId}')" title="Hapus">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
...
function updateCVStatus() {
    ...
    if (appData.cv && appData.cv.file) {
        let uploadedAt = 'Baru saja';
        if (appData.cv.uploadedAt && appData.cv.uploadedAt.toDate) {
            uploadedAt = appData.cv.uploadedAt.toDate().toLocaleString('id-ID');
        }
        
        const fileName = appData.cv.name || appData.cv.file.name || 'cv.pdf';
        const onlineStatus = FirebaseHelper.isOnline() ? '🟢 Online' : '🟡 Offline';
        
        cvStatus.innerHTML = `
            <div class="alert alert-success" style="color: var(--success); margin-top: 1rem;">
                <i class="fas fa-check-circle"></i> CV sudah diupload: ${fileName}
                <br><small style="color: var(--text-muted);">Upload: ${uploadedAt}</small>
                <br><small style="color: var(--text-muted);">Status: ${onlineStatus}</small>
            </div>
        `;
    } else {
        cvStatus.innerHTML = `
...
function showTaskDetail(taskId) {
    ...
    if (task.pdfFile || task.pptFile || task.videoFile) {
        ...
        if (task.videoFile && task.videoFile.url) {
            const videoTitle = task.title ? task.title.replace(/'/g, "\\'") : '';
            const videoName = task.videoFile.name || 'video.mp4';
            const videoUrl = task.videoFile.url;
            
            fileLinksHtml += `
                <button onclick="openVideoPlayer('${videoUrl}', '${videoName}', '${videoTitle}')" 
                ...
                <a href="${videoUrl}" download class="file-link" style="padding: 0.5rem 1rem; background: var(--bg-secondary); color: var(--text-primary); border-radius: var(--radius-sm); text-decoration: none; display: inline-flex; align-items: center; gap: 0.25rem;">
    ...
    modalBody.innerHTML = `
        <div class="task-detail-content">
            ...
            <ul style="list-style: none; padding: 0; color: var(--text-secondary);">
                <li style="margin: 0.25rem 0;"><strong>Mata Kuliah:</strong> ${course}</li>
                <li style="margin: 0.25rem 0;"><strong>Kategori:</strong> ${category}</li>
...
function showCertificateDetail(certId) {
    ...
    if (cert.file && cert.file.url) {
        if (cert.file.type && cert.file.type.startsWith('image/')) {
            ...
            const fileName = cert.file.name || 'document.pdf';
            previewHtml = `
    ...
    modalBody.innerHTML = `
        <div class="certificate-preview" style="text-align: center;">
            ...
            <p style="color: var(--text-secondary);">Diterbitkan oleh: ${issuer}</p>
            <p style="color: var(--text-muted);">Tahun: ${year}</p>
        </div>
...
function openVideoPlayer(videoUrl, fileName, title) {
    ...
    videoPlayer.onloadeddata = function() {
        ...
        if (videoInfo) {
            const duration = formatVideoDuration(videoPlayer.duration);
            videoInfo.innerHTML = `
...
// ============================================
// GLOBAL FUNCTIONS EXPORT (untuk onclick di HTML)
// ============================================
